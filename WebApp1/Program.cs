...
var connectionString = builder.Configuration.GetConnectionString("MarketManagement");

builder.Services.AddDbContext<AccountContext>(options => options.UseSqlServer(connectionString));

builder.Services.AddDbContext<MarketContext>(options => options.UseSqlServer(connectionString));
...
// map the interface and repository
